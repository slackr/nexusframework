...
abstract class Nexus {
    ...
    public function start() {
        ...
        if ($load_chan) {
            $this->disp_msg($load_chan);
        }
        
        /**
         * once we have proper values for user_file/chan_file we can prompt
         */
        if ($this->prompt_for_adduser) {
            $this->user_add_prompt();
            exit();
        }
                ...
                if ($this->client['servers'][0]['ip']) {
                    ...
                    if ($this->fork_possible && $this->fork) {
                        ...
                        if ($p > 0) {
                            ...
                            $this->disp_error("failed to launch into the background");
                        }
                    }
                    /**
                     ...
                     */
                    if ($this->client['save_data']) {
                        $this->timer('save_data', 'write_all()', $this->client['save_data'] * 60, true);
                    }
    ...
    protected function connect() {
        ...
        while ($this->client['servers'][$snum]['ip']) {
            $this->sock = @stream_socket_client("tcp://" . $this->client['servers'][$snum]['ip'] . ":" . $this->client['servers'][$snum]['port'], $err_no, $err_str, 15, STREAM_CLIENT_CONNECT, $context);
            if (!$this->sock) {
            ...
            if ($this->client['servers'][$snum]['pass'] != '') {
                $this->send("PASS " . $this->client['servers'][$snum]['pass']);
            }
    ...
    protected function load_config($configfile) {
        if (!$fp = fopen($configfile, 'r')) {
            return "could not open file: $configfile";
        }
        $line = 0;
        $chan = "";
        while (!feof($fp)) {
                ...
                switch ($this->gettok($data, 1)) {
                    case "save_data":
                        if (is_numeric($this->gettok($data, 2)) && $this->gettok($data, 2) > $this->min_save_period) {
                            ...
                            $error = "invalid 'savedata'";
                        }
                        break;
                    case "server":
                        if (preg_match('/^(server)\s[a-z\d.-]+\s[\d]{2,5}.*/si', $data)) {
                            $this->client['servers'][] = array(
                                'ip' => $this->gettok($data, 2),
                    ...
                    case "channel":
                        if (!$open_brace) {
                            ...
                            if (preg_match("/^#/si", $chan)) {
                                ...
                                $error = "invalid channel container";
                            }
                        } else {
                            $error = "previous container not closed";
                        }
                        break;
                    ...
                    default:
                        ...
                        if (preg_match("/^[\w0-9\-._]+$/si", $setting)) {
                            for ($x = 2; ($tmp = $this->gettok($data, $x)) != ''; $x++) {
                                $val .= $tmp . " ";
                            }
                            $val = rtrim($val, " ");
                            if ($this->config_regex[$setting] && !preg_match($this->config_regex[$setting], $val)) {
                                ...
                                if ($open_brace && $chan) {
                                    $this->settings[$chan][$setting] = $val;
                                } else {
                                    $this->client[$setting] = $val;
                                }
                            }
                        }
                        break;
        ...
        fclose($fp);
        if (!$error && $open_brace) {
            $error = "container not closed";
        }
        if ($error) {
            return $error . " in $configfile (line: $line)";
        }
        if ($ch_count > 0) {
            $this->disp_msg("loaded $ch_count channels from $configfile");
        }
        return 1;
    }
    ...
    private function load_userfile($file) {
        if (!$fp = @fopen($file, 'r')) {
            return "could not open user file: $file";
        }
        $line  = 0;
        $count = 0;
        while (!feof($fp)) {
            ...
            if ($data) { // don't test blank lines
                if (!preg_match("/[\w-\d^`{}]+/si", $this->gettok($data, 1)) || !preg_match("/.+@.+/si", $this->gettok($data, 2)) || !preg_match("/[" . $this->userflags . "]/si", $this->gettok($data, 3)) || !preg_match("/" . $this->regex_crypt . "/si", $this->gettok($data, 4))) {
                    $this->disp_msg("warning: skipped invalid entry in user file ($file:$line)");
                } else {
                    $this->users[$this->gettok($data, 1)] = array(
                        'mask' => $this->gettok($data, 2),
    ...
    private function load_chanfile($file) {
        if (!$fp = @fopen($file, 'r')) {
            return "could not open chan file: $file";
        }
        $line  = 0;
        $count = 0;
        while (!feof($fp)) {
            ...
            if ($data != "") {
                ...
                if (preg_match("/^#/si", $ch) && !isset($this->channels[$ch]['static'])) {
                    ...
                    for ($x = 1; $set = $this->gettok($token, $x, ";"); $x++) {
                        ...
                        if ($val != "") {
                            $this->settings[$ch][$key] = (!is_numeric($val) ? $this->rdecrypt($val) : $val);
                        }
                    }
                    $count++;
                } else {
                    $this->disp_msg("warning: skipped invalid entry in channel file ($file:$line)");
                }
            }
        }
        fclose($fp);
    ...
    protected function rdecrypt($str) {
        $tmp     = "";
        $decrypt = "";
        for ($x = 1; $tmp = $this->gettok($str, $x, "_"); $x++) {
            $decrypt .= chr(hexdec($tmp));
        }
        return $decrypt;
    }
    ...
    protected function ikey_exists($needle, $haystack) {
        foreach (array_keys($haystack) as $key) {
            if (preg_match("/^" . preg_quote($needle) . "$/si", $key)) {
                return true;
            }
        }
        return false;
    ...
    public function disp_raw($data) {
        if ($data && !preg_match("/^PING /si", $data) && $this->client['debug_raw']) {
            $this->disp_msg($data, "raw");
        }
    }
        ...
        switch ($this->gettok($this->raw, 2)) {
            ...
            case "MODE":
                ...
                if (preg_match("/^#.*/", $this->gettok($this->raw, 3))) {
                    ...
                    for ($z = 5; $this->gettok($this->raw, $z); $z++) {
                        $p .= $this->gettok($this->raw, $z) . " ";
                    }
                ...
                default:
                    switch ($mode[$x]) {
                        case "o":
                            ...
                            if ($ud) {
                                $this->pre_on_op($nick, $host, $chan, $onick);
                                $this->on_op($nick, $host, $chan, $onick);
                            } else {
                                $this->pre_on_deop($nick, $host, $chan, $onick);
                                $this->on_deop($nick, $host, $chan, $onick);
                            }
                            break;
                        case "v":
                            $vnick = $this->gettok($params, $step);
                            if ($ud) {
                                $this->pre_on_vo($nick, $host, $chan, $vnick);
                                $this->on_vo($nick, $host, $chan, $vnick);
                            } else {
                                $this->pre_on_devo($nick, $host, $chan, $vnick);
                                $this->on_devo($nick, $host, $chan, $vnick);
                            }
                            break;
                        case "h":
                            $hnick = $this->gettok($params, $step);
                            if ($ud) {
                                $this->pre_on_dehop($nick, $host, $chan, $hnick);
                                $this->on_hop($nick, $host, $chan, $hnick);
                            } else {
                                $this->pre_on_dehop($nick, $host, $chan, $hnick);
                                $this->on_dehop($nick, $host, $chan, $hnick);
                            }
                            break;
                        case "b":
                            $mask = $this->gettok($params, $step);
                            if ($ud) {
                                $this->pre_on_ban($nick, $host, $chan, $mask);
                                $this->on_ban($nick, $host, $chan, $mask);
                            } else {
                                $this->pre_on_unban($nick, $host, $chan, $mask);
                                $this->on_unban($nick, $host, $chan, $mask);
                            }
                            break;
                        case "k":
                            $key = $this->gettok($params, $step);
                            if ($ud) {
                                $this->pre_on_key($nick, $host, $chan, $key);
                                $this->on_key($nick, $host, $chan, $key);
                            } else {
                                $this->pre_on_unkey($nick, $host, $chan, $key);
                                $this->on_unkey($nick, $host, $chan, $key);
                            }
                            break;
                        default:
                            $par = $this->gettok($params, $step);
                            if ($ud) {
                                $this->pre_on_mode($nick, $host, $chan, $mode[$x], $par);
                                $this->on_mode($nick, $host, $chan, $mode[$x], $par);
                            } else {
                                $this->pre_on_demode($nick, $host, $chan, $mode[$x], $par);
                                $this->on_demode($nick, $host, $chan, $mode[$x], $par);
                            }
                            break;
                    }
                    // only step ahead if the mode has a parameter
                    if (preg_match("/[ovhklbIe]/", $mode[$x])) {
                        $step++;
                    }
                    break;
            }
    ...
    protected function cmd_op($chan, $nick, $op = "o") {
        ...
        if (is_array($nick)) {
            ...
            for ($x = 0; ($nick[$x] && $x <= 16); $x++) {
                $opline .= $nick[$x] . " ";
                if (!(($x + 1) % 4) && isset($nick[$x + 1])) {
                    $opline .= "\r\nMODE $chan +" . str_repeat($op, 4) . " ";
                }
            }
            $this->send(rtrim($opline, "\r\n"));
        } else if (!$this->isop($nick, $chan)) {
            $this->send("mode $chan +$op $nick");
        }
    }
    ...
    private function rawnum($num) {
        switch ($num) {
            ...
            case "473":
                ...
                if (!$this->is_timer("j_retry_$safe")) {
                    $this->timer("j_retry_$safe", 'send("JOIN ' . $chan . '")', $this->retry_time, true);
                }
                break;
            case "475":
                $chan = $this->gettok($this->raw, 4);
     ...
     * but can be any channel status indicator that prefixes /names
     * 
     * @param string $ nickname to check
     * @param string $ channel to check for status of nickname
     * @param string $ the default is
     * @return bool 
     */
    ...
    private function pre_on_nick($nick, $host, $newnick) {
        if ($nick == $this->me) {
            ...
            if ($newnick == $this->keepnick) {
                $this->kill_timer('keepnick');
            }
            $this->me = $newnick;
        } else {
    ...
    private function pre_on_op($nick, $host, $chan, $onick) {
        if (!$this->isop($onick, $chan)) {
            $this->chans[$chan][$onick]['status'] .= '@';
        }
    }
    ...
    private function pre_on_vo($nick, $host, $chan, $vnick) {
        if (!$this->isop($vnick, $chan, '+')) {
            $this->chans[$chan][$vnick]['status'] .= '+';
        }
    }
    ...
    private function pre_on_hop($nick, $host, $chan, $hnick) {
        if (!$this->isop($hnick, $chan, '%')) {
            $this->chans[$chan][$hnick]['status'] .= '%';
        }
    }
    ...
    private function pre_on_part($nick, $host, $chan) {
        if ($nick == $this->me) {
            ...
            unset($this->chans[$chan][$nick]);
        }
    }
    
    private function pre_on_kick($nick, $host, $chan, $knick, $reason) {
        if (!$this->chans[$chan][$nick]['host']) {
            $this->chans[$chan][$nick]['host'] = $host;
        }
        if ($knick == $this->me) {
            unset($this->chans[$chan]);
            $this->timer('k_rejoin_' . $this->safe_name($chan), 'cmd_join("' . $chan . '")', 5);
        } else {
            unset($this->chans[$chan][$knick]);
        }
    }
    
    private function pre_on_quit($nick, $host, $reason) {
        if (isset($this->ident[$nick])) {
            unset($this->ident[$nick]);
        }
        foreach ($this->chans as &$key) {
    ...
    private function pre_on_privmsg($nick, $host, $text) {
        ...
        if (!$this->is_timer('priv_command_throttle')) {
            if ($text == "\001VERSION\001" && $this->version_reply) {
                $this->timer('priv_command_throttle', null, 10);
