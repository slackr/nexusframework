...
class Phobos extends Nexus {
    ...
    protected function on_init() {
        ...
        if ($load_seen != 1) {
            $this->disp_msg($load_seen);
        }
        
        /* spawn timer to save seen file
         */
        if ($this->client['save_data']) {
            $this->timer('save_seen_file', 'write_seenfile()', $this->client['save_data'] * 60, true);
        }
    }
    ...
    private function load_seenfile($file) {
        if (!$fp = @fopen($file, 'r')) {
            return "could not open seen file: $file";
        }
        $line    = 0;
        $count   = 0;
        $skipped = 0;
        while (!feof($fp)) {
            ...
            if ($data) { //don't test blank lines
                if (!preg_match("/^.+[@].+$/si", $this->gettok($data, 2)) || !preg_match("/^[0-9]+$/si", $this->gettok($data, 3))) {
                    ...
                    $this->seen_list[$tmp_splitseen[0]] = array(
                        'host' => $tmp_splitseen[1],
    ...
    protected function on_deop($nick, $host, $chan, $onick) {
        if ($this->settings[$chan]['massdeop'] == 1 && !$this->has_flag('p', $nick) && $this->me != $nick && $this->me != $onick && $this->isop($this->me, $chan)) {
    ...
    protected function on_join($nick, $host, $chan) {
        if ($this->isop($this->me, $chan)) {
            if ($this->settings[$chan]['joinflood'] == 1 && !$this->has_flag('p', $nick) && $this->me != $nick) {
        ...
        if ($this->me != $nick) {
            if ($this->ikey_exists($nick, $this->ping_notify) && $this->client['ping_notify'] == 1) {
                $tmp_pingedby = "";
    ...
    protected function on_kick($nick, $host, $chan, $knick, $reason) {
        if ($this->settings[$chan]['kickflood'] == 1 && !$this->has_flag('p', $nick) && $this->me != $nick && $this->me != $knick && $this->isop($this->me, $chan)) {
    ...
    protected function on_nick($nick, $host, $newnick) {
        if ($this->me != $nick) {
            if ($this->ikey_exists($newnick, $this->ping_notify) && $this->client['ping_notify'] == 1) {
                $tmp_pingedby = "";
    ...
    protected function on_quit($nick, $host, $reason) {
        foreach ($this->chans as $key => $val) {
            if ($this->me != $nick) {
                if (sizeof($this->chans[$key]) == 1 && !$this->isop($this->me, $key)) {
                    $this->send("PART $key\r\nJOIN $key");
                }
    ...
    protected function on_pubmsg($nick, $host, $chan, $text) {
        if (($this->isop($nick, $chan) && $this->settings[$chan]['ignoreops']) || ($this->isop($nick, $chan, '+') && $this->settings[$chan]['ignorevoices']) || ($this->isop($nick, $chan, '%') && $this->settings[$chan]['ignorehops'])) {
            $noflood = true;
        }
        
        if ($this->settings[$chan]['pubflood'] == 1 && !$this->has_flag('p', $nick) && $this->me != $nick && $this->isop($this->me, $chan) && !$noflood) {
        ...
        if ($text[0] == $this->client['cmd_char'] && isset($this->ident[$nick]) && $this->has_flag('c', $nick) && !$this->is_timer('pub_command_throttle')) {
            ...
            switch ($cmd) {
                ...
                case ($cmd == "up" || $cmd == "op"):
                    ...
                    if ($this->has_flag('v', $nick)) {
                        $mode = 'v';
                    }
                    if ($this->has_flag('h', $nick)) {
                        $mode = 'h';
                    }
                    if ($this->has_flag('o', $nick)) {
                        $mode = 'o';
                    }
                    if ($mode && !$this->isop($nick, $chan, $ovh[$mode]) && $this->isop($this->me, $chan)) {
                ...
                case ($cmd == "kick" || $cmd == "k"):
                    if ($this->isop($this->me, $chan)) {
                        for ($x = 3; $this->gettok($text, $x); $x++) {
                            $reason .= $this->gettok($text, $x) . " ";
                        }
                        foreach ($this->chans[$chan] as $key => $val) {
                            if (($this->iswm($this->gettok($text, 2), $key) || $this->iswm($this->gettok($text, 2), $val['host'])) && $this->me != $key) {
                ...
                case ($cmd == "write" || $cmd == "wrt"):
                    if ($this->has_flag('a', $nick)) {
                        if (!$this->is_timer('pub_wrt_throttle')) {
                            ...
                            $this->send("PRIVMSG $chan :please wait " . $this->timers['pub_wrt_throttle']->get_timeleft() . "s before issuing another write command");
                        }
                    }
                    break;
            }
                    ...
                    if (!$seen_found) {
                        foreach ($this->chans as $key => $val) {
                            if ($this->ikey_exists($tmp_seenwho, $this->chans[$key])) {
                                if ($this->client['seen_notifyuser'] == 1 && !$this->ikey_exists($tmp_seenwho, $this->chans[$chan])) {
        ...
        if (!$this->is_timer('everyone_command_throttle')) {
            if ((($tmp_reg_pingmatch = preg_match('/^\s*([\w_^`\\{}\[\]|-]+)([:;,+\s!]\s?)+ping.*$/si', $text)) > 0 || ($tmp_reverse_pingmatch = preg_match('/^\s*ping([:;,+\s!]\s?)+([\w_^`\\{}\[\]|-]+).*$/si', $text)) > 0) && $this->client['ping_notify'] == 1) {
                $this->timer('everyone_command_throttle', null, 3);
                    ...
                    if ($this->has_flag('j', $nick)) {
                        ...
                        if ($ch[0] == '#') {
                            if (!isset($this->settings[$ch])) {
                                ...
                                if ($ky) {
                                    $this->settings[$ch]['key'] = $ky;
                                }
                                if (!isset($this->chans[$ch])) {
                                    $this->cmd_join($ch);
                                }
                            } else {
                                $this->send("PRIVMSG $nick :channel already exists '$ch'");
                            }
                        } else {
                            $this->send("PRIVMSG $nick :invalid channel '$ch'");
                        }
                    }
                    break;
                case "join":
                    if ($this->has_flag('j', $nick)) {
                        ...
                        if ($ch[0] == '#') {
                            if (!isset($this->chans[$ch])) {
                                ...
                                $this->send("PRIVMSG $nick :i'm already on '$ch'");
                            }
                        } else {
                            $this->send("PRIVMSG $nick :invalid channel '$ch'");
                        }
                        ...
                        if ($ch[0] == '#') {
                            if (!$this->settings[$ch]['static']) {
                                if (isset($this->settings[$ch])) {
                                    ...
                                    if (isset($this->chans[$ch])) {
                                        $this->send("PART $ch :-chan");
                                    }
                                    $this->send("PRIVMSG $nick :'$ch' was removed from memory");
                                } else {
                                    $this->send("PRIVMSG $nick :'$ch' is not in memory");
                                }
                            } else {
                                $this->send("PRIVMSG $nick :cannot remove static channel '$ch'");
                            }
                        } else {
                            $this->send("PRIVMSG $nick :invalid channel '$ch'");
                        }
                    ...
                    if ($this->has_flag('j', $nick)) {
                        ...
                        if ($ch[0] == '#') {
                            if (isset($this->chans[$ch])) {
                                ...
                                $this->send("PRIVMSG $nick :i'm not on '$ch'");
                            }
                        } else {
                            $this->send("PRIVMSG $nick :invalid channel '$ch'");
                        }
    ...
    protected function ping_notify_update_record($pingnick, $pinger, $chan) {
        if (sizeof($this->ping_notify[$pingnick]) < $this->max_ping_notify_records && $this->client['ping_notify'] == 1) {
